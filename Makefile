...
INCLUDE += -I../../util/
INCLUDE += -I./MyIncludes/
INCLUDE += -I./SysIncludes/
INCLUDE += -I./Drivers/
INCLUDE += -I./
...
# Linker file / options
LFLAGS  = -T $(wildcard $(BLD)*.ld)
LFLAGS += --specs=nano.specs --specs=nosys.specs 
LFLAGS += -Wl,-Map -Wl,$(PROJECT).map


# Object files to be created
OBJECTS = $(patsubst %.c, %.o, $(wildcard *.c)) $(patsubst %.s, %.o, $(wildcard *.s)) $(patsubst $(DRV)%.c, $(DRV)%.o, $(wildcard $(DRV)*.c))
OBJECTS += $(patsubst $(BLD)%.c, $(BLD)%.o, $(wildcard $(BLD)*.c))  $(patsubst $(BLD)%.s, $(BLD)%.o, $(wildcard $(BLD)*.s))
